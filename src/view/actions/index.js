...
export const taskNew = ({ task, top }) => (dispatch, getState) => {
    ...
    db
        .post({ task: task, done: false, index })
...
export const taskChangeText = task => dispatch => {
    db.put(task.toJS()).then(_ => dispatch(tasksRefresh()));
};
...
export const taskDelete = task => dispatch => {
    db
        .remove(task.get('_id'), task.get('_rev'))
        .then(_ => dispatch(tasksRefresh()));
};

export const tasksRefresh = _ => dispatch => {
    var i = 0;
    db.allDocs({ attachments: true, include_docs: true }).then(({ rows }) => {
